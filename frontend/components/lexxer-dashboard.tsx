...
export function LexxerDashboard() {
  ...
  // Fetch the full trace for the selected run whenever its id changes
  // (initial auto-selection and every click). The runs list only returns
  // summaries without events, so without this the timeline would show
  ...
  useEffect(() => {
    ...
    getRun(selected.id)
      ...
      .catch(() => { if (!cancelled) setError(`Could not load details for run ${selected.id}.`) })
  ...
  return <main className="app-shell">
    ...
    <div className="app-content">
      <div className="dashboard-column">
        ...
        <LexxerTrace run={selected} loading={loading || detailLoading} error={error} onRetry={loadDashboard} /><LexxerArchitecture />
      </div>
